...
protected:
	...
	void draw() final override {
		...
		if (OVR_SUCCESS(ovr_GetInputState(_session, ovrControllerType_Touch, &inputState))) {
			...
			updateTriggers(inputState.IndexTrigger[ovrHand_Left] > 0.5f,
				inputState.IndexTrigger[ovrHand_Right] > 0.5f);
			if (inputState.IndexTrigger[ovrHand_Left] > 0.5f && inputState.IndexTrigger[ovrHand_Right] > 0.5f) {
			}
		}
...
static float rightY;
static float rightZ;
static float rightOrientX;
static float rightOrientY;
static float rightOrientZ;
static float leftOrientX;
...
class CO2App : public RiftApp {
	std::shared_ptr<Co2Scene> co2Scene;
...
protected:
	...
	void updateTriggers(bool left, bool right) {
		leftHandTriggerPressed = left;
		if (left) {
			co2Scene->lineLeft->colorRed();
		}
		else co2Scene->lineLeft->colorGreen();
		rightHandTriggerPressed = right;
		if (right) {
			co2Scene->lineRight->colorRed();
		}
		else co2Scene->lineRight->colorGreen();
	}
